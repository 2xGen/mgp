...
import { Loader2, Terminal, UserCheck, Trash2, UserPlus, Zap, Eye } from "lucide-react";
import { 
    fetchLocations, 
    fetchAccounts, 
...
export default function ManageTeamPage() {
  ...
  const hasReachedLimit = useMemo(() => {
    return teamMemberLimit > 0 && teamInvites.length >= teamMemberLimit;
  ...
  const getInitialData = useCallback(async () => {
    ...
    try {
        ...
        for (const account of fetchedAccounts) {
            ...
            if (adminsRes.data?.admins) {
                adminsRes.data.admins.forEach((admin: any) => {
                    ...
                    const adminEmail = admin.admin.split('/')[1]; // assuming format like users/email@example.com
                    if (!usersMap.has(adminIdentifier) && admin.admin.split('/')[1] !== user.email) {
                         usersMap.set(adminIdentifier, {
                            email: adminEmail,
                            name: admin.displayName || adminEmail,
                            role: admin.role,
        ...
        // Update the `managedLocations` state based on all locations found
  ...
  const existingInvitesMap = useMemo(() => {
    ...
    teamInvites.forEach(inv => map.set(inv.name, inv)); // Assuming name is unique for now
    return map;
  }, [teamInvites]);
                    ...
                    <TableBody>
                        {allGoogleUsers.length > 0 ? allGoogleUsers.map((gUser) => {
                            ...
                            if (gUser.isPending) {
                                ...
                                statusBadge = <Badge className="bg-blue-100 text-blue-800 border-blue-200">Invite Created</Badge>;
                            } else {
                                statusBadge = <Badge variant="secondary">Google User</Badge>;
                            }

                            return (
                                <TableRow key={gUser.email}>
                                    ...
                                    <TableCell className="text-right">
                                        {!gUser.isPending && !existingInvite && (
                                            <InviteUserForm 
                                                trigger={
